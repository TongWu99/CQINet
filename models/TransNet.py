...
class Transformer(nn.Module):
    ...
    def __init__(self, d_model: int = 64, nhead: int = 8, num_encoder_layers: int = 6,
        ...
        # CSI 输入大小: 32×52×2 = 3328
        self.csi_size = 32 * 52 * 2
        assert not (self.csi_size % self.d_model), 'd_model needs to be divisible by the size of the entire csi matrix (3328)'
        self.feature_shape = (self.csi_size // self.d_model, self.d_model)
        ...
        # 支持近似压缩率
        target_compressed_dim = self.csi_size // reduction
        self.compressed_dim = max(1, target_compressed_dim)
        self.actual_reduction = self.csi_size / self.compressed_dim
        
        self.fc_encoder = nn.Linear(self.csi_size, self.compressed_dim)
        self.fc_decoder = nn.Linear(self.compressed_dim, self.csi_size)
        self._reset_parameters()
